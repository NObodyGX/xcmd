...
from .base import Header, Mp4Box, Mp4FullBox

from ..m4a_data import (
    mp4ra_table,
    es_table,
    sound_codec_table,
    freq_table,
...
class Avc1Box(Mp4FullBox):
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            self.box_info["compressorname"] = fp.read(compressorname_size).decode(
                "utf-8", errors="ignore"
            )
...
class DvheBox(Avc1Box):
    pass


class Dvh1Box(Avc1Box):
    pass


class DvavBox(Avc1Box):
    pass


class Dva1Box(Avc1Box):
    pass


class Hvc1Box(Avc1Box):
    pass


class Hev1Box(Avc1Box):
    pass


class Av01Box(Avc1Box):
    pass


class Avc4Box(Avc1Box):
    pass


class Avc3Box(Avc1Box):
    pass


class Avc2Box(Avc1Box):
    pass


class Mp4vBox(Avc1Box):
    pass


class EncvBox(Avc1Box):
    pass
        ...
        try:
            ...
            for i in range(self.box_info["numOfSequenceParameterSets"]):
                ...
                sequence_param = {
                    ...
                    "sequenceParameterSetNALUnit": binascii.b2a_hex(
                        fp.read(spsl)
                    ).decode("utf-8"),
                }
            ...
            for i in range(self.box_info["numOfPictureParameterSets"]):
                ...
                picture_param = {
                    ...
                    "pictureParameterSetNALUnit": binascii.b2a_hex(
                        fp.read(ppsl)
                    ).decode("utf-8"),
                }
            ...
            if (
                ...
                for i in range(self.box_info["numOfSequenceParameterSetExtLength"]):
                    ...
                    sequence_param = {
                        ...
                        "sequenceParameterSetExtNALUnit": binascii.b2a_hex(
                            fp.read(spse)
                        ).decode("utf-8"),
                    }
...
class HvccBox(Mp4Box):
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            self.box_info["general_profile_compatibility_flags"] = "{0:#010x}".format(
                read_u32(fp)
            )
            self.box_info["general_constraint_indicator_flags"] = (
                "0x" + binascii.b2a_hex(fp.read(6)).decode("utf-8")
            )
...
class Av1cBox(Mp4Box):
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            self.box_info["initial_presentation_delay_present"] = (
                fourth_byte >> 4
            ) & 0x01
            ...
            self.box_info["configOBUs"] = binascii.b2a_hex(fp.read(bytes_left)).decode(
                "utf-8"
            )
        finally:
            fp.seek(self.start_of_box + self.size)
            ...
            while fp.tell() < (self.start_of_box + self.size):
                ...
                if this_descriptor_dict["tag_id"] == 4:
                    ...
                    this_descriptor_dict["mp4ra_registered_type"] = (
                        mp4ra_table[mp4ra_type]
                        if mp4ra_type in mp4ra_table
                        else mp4ra_type
                    )
                    type_byte = int.from_bytes(payload[1:2], byteorder="big")
                    es_type = type_byte >> 2
                    this_descriptor_dict["es_type"] = es_table.get(es_type, es_type)
                    this_descriptor_dict["upstream_flag"] = type_byte & 2
                    this_descriptor_dict["specific_info_flag"] = type_byte & 1
                    this_descriptor_dict["buffer_size"] = int.from_bytes(
                        payload[2:5], byteorder="big"
                    )
                    this_descriptor_dict["max_bitrate"] = int.from_bytes(
                        payload[5:9], byteorder="big"
                    )
                    this_descriptor_dict["avg_bitrate"] = int.from_bytes(
                        payload[9:13], byteorder="big"
                    )
                    if this_descriptor_dict["specific_info_flag"]:
                        # decoder-specific descriptor embedded in es descriptor
                        decoder_specific_dict = {
                            "tag_id": int.from_bytes(payload[13:14], byteorder="big")
                        }
                        padding = 0x0
                        b = 14
                        while (
                            int.from_bytes(payload[b : b + 1], byteorder="big") == 0x80
                        ):
                            padding = (padding << 8) + 0x80
                            b += 1
                        decoder_specific_dict["preamble"] = hex(padding)
                        decoder_specific_dict["payload_length"] = int.from_bytes(
                            payload[b : b + 1], byteorder="big"
                        )
                        if es_type == 5:
                            two_bytes = int.from_bytes(
                                payload[b + 1 : b + 3], byteorder="big"
                            )
                            field_size = 5
                            ...
                            if sound_codec == 31:
                                field_size = 6
                                sound_codec = (
                                    (two_bytes >> (16 - bits_read - field_size)) & 63
                                ) + 32
                                bits_read += field_size
                            decoder_specific_dict["sound_codec"] = sound_codec_table[
                                sound_codec
                            ]
                            field_size = 4
                            decoder_specific_dict["freq_id"] = freq_table[
                            ...
                            if decoder_specific_dict["freq_id"] == 15:
                                ...
                                four_bytes = int.from_bytes(
                                    payload[b + 2 : b + 6], byteorder="big"
                                )
                                field_size = 24
                                # subtract 8 from bits_read because we are not including first byte in payload
                                bits_read -= 8
                                decoder_specific_dict["freq_value"] = (
                                    four_bytes >> (32 - bits_read - field_size)
                                    & 16777215
                                )
                                # adjust to two_bytes
                                bits_read += field_size - 16
                                two_bytes = int.from_bytes(
                                    payload[b + 4 : b + 6], byteorder="big"
                                )
                            field_size = 4
                            sound_chan = (
                                two_bytes >> (16 - bits_read - field_size)
                            ) & 15
                            decoder_specific_dict["channels"] = sound_channel_table.get(
                                sound_chan, sound_chan
                            )
                        else:
                            decoder_specific_dict["payload"] = binascii.b2a_hex(
                                payload[b + 1 :]
                            ).decode("utf-8")
                        this_descriptor_dict["decoder_specific_descriptor"] = (
                            decoder_specific_dict
                        )
                else:
                    this_descriptor_dict["payload"] = binascii.b2a_hex(payload).decode(
                        "utf-8"
                    )
...
class DataBox(Mp4FullBox):
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            fp.seek(4, 1)
            self.box_info["text"] = fp.read(
                self.size - (self.header.header_size + 8)
            ).decode("utf-8")
        finally:
            fp.seek(self.start_of_box + self.size)
        ...
        try:
            ...
            if self.box_info["system_id"] == "1077efecc0b24d02ace33c1e52e2fb4b":
                ...
                for i in range(self.box_info["key_count"]):
                    self.box_info["key_list"].append(
                        binascii.b2a_hex(fp.read(16)).decode("utf-8")
                    )
        finally:
            fp.seek(self.start_of_box + self.size)
...
class TencBox(Mp4FullBox):
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            if (
                self.box_info["default_isProtected"] == 1
                and self.box_info["default_Per_Sample_IV_Size"] == 0
            ):
        ...
        try:
            ...
            while bytes_left > 7:
                ...
                if not current_header.type.isprintable():
                    current_header.type = "#{0:#d}".format(
                        struct.unpack(">I", current_header.type.encode("utf-8"))[0]
                    )
...
class XyzBox(Mp4Box):
    ...
    def __init__(self, fp, header, parent):
        ...
        try:
            ...
            self.box_info["data"] = fp.read(self.box_info["data_size"]).decode(
                "utf-8", errors="ignore"
            )
        finally:
            fp.seek(self.start_of_box + self.size)
