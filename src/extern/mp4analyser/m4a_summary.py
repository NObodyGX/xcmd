...
class Summary(object):
    ...
    def __init__(self, mp4file):
        ...
        if [box for box in boxes if box.type == "ftyp" or box.type == "styp"]:
            fstyp = [box for box in boxes if box.type == "ftyp" or box.type == "styp"][
                0
            ]
        ...
        if [box for box in boxes if box.type == "moov"]:
            ...
            self.data["duration (secs)"] = round(
                mvhd.box_info["duration"] / mvhd.box_info["timescale"]
            )
            if self.data["duration (secs)"] > 0:
                self.data["bitrate (bps)"] = round(
                    8 * self.data["filesize (bytes)"] / self.data["duration (secs)"]
                )
            ...
            for trak in traks:
                this_trak = {}
                this_trak["track_id"] = [
                    box for box in trak.children if box.type == "tkhd"
                ][0].box_info["track_ID"]
                ...
                stbl = [
                    box
                    for box in [box for box in mdia.children if box.type == "minf"][
                        0
                    ].children
                    if box.type == "stbl"
                ][0]
                ...
                if sz.box_info["sample_size"] > 0:
                    ...
                    trak_size = sum(
                        [entry["entry_size"] for entry in sz.box_info["entry_list"]]
                    )
                ...
                codec_info = (
                    [box for box in stbl.children if box.type == "stsd"][0]
                ).children[0]
                ...
                if media == "vide":
                    ...
                    this_trak["width"] = (
                        codec_info.box_info["width"]
                        if "width" in codec_info.box_info
                        else "unknown"
                    )
                    this_trak["height"] = (
                        codec_info.box_info["height"]
                        if "height" in codec_info.box_info
                        else "unknown"
                    )
